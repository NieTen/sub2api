export default {
    accounts: {
      ...
      dataImportHint: '支持多文件导入、直接粘贴 JSON、拖拽文件进入；系统会自动识别 CPA 或 sub2api 结构并合并为可直接导入的平台数据。',
      dataImportWarning: '这里只处理账号导入；会创建新账号，分组仍需手工绑定，请确认数据不会和现有账号冲突。',
      dataImportFile: '导入文件',
      dataImportButton: '直接导入',
      dataImporting: '导入中...',
      dataImportSelectFile: '请选择 JSON 文件',
      dataImportParseFailed: '文本解析失败',
      dataImportParseFailedFile: '文件 {name} 解析失败',
      dataImportInvalidFile: '文件 {name} 不是受支持的账号导入文件',
      dataImportIgnoredFiles: '已忽略 {count} 个非 JSON 文件',
      dataImportFailed: '账号导入失败',
      dataImportText: '直接粘贴 JSON',
      dataImportPlaceholder: '支持单个对象、数组、NDJSON、连续多个 JSON 对象，或粘贴 sub2api 导入包。',
      dataImportTextHint: '可粘贴 CPA auth JSON、sub2api 导入包，或多个 JSON 对象；系统会自动转换为账号导入包。',
      dataImportTextSource: '文本输入',
      dataImportDropHint: '支持拖拽多个 .json 文件，文件只在当前浏览器会话中读取。',
      dataImportClear: '清空',
      dataImportOutputTitle: '生成结果',
      dataImportOutputHint: '下方内容就是可直接导入平台的 sub2api 数据包，也可复制或下载。',
      dataImportCopy: '复制 JSON',
      dataImportDownload: '下载 JSON',
      dataImportInputRecords: '输入记录',
      dataImportAccounts: '账号数',
      dataImportProxies: '代理数',
      dataImportSkipped: '跳过',
      dataImportPreviewTitle: '账号预览',
      dataImportPreviewEmpty: '等待输入内容',
      dataImportSource: '来源',
      dataImportSourceCpa: 'CPA',
      dataImportSourceSub2Api: 'sub2api',
      dataImportIssues: '转换提示',
      dataImportOutputPlaceholder: '解析结果会显示在这里',
      dataImportCopied: 'JSON 已复制到剪贴板',
      ...
      batchTest: {
        ...
        start: '开始测试',
        running: '测试中...',
        runningWithConcurrency: '正在以 15 并发执行批量连接测试...',
        ready: '点击开始测试后，系统会多线程并发测试选中的账号。',
        successCount: '成功 {count}',
        failedCount: '失败 {count}',
        completed: '批量测试完成，成功 {count} 个',
        partial: '批量测试部分完成：成功 {success} 个，失败 {failed} 个',
        failed: '批量测试连接失败',
        columns: {
          account: '账号',
          latency: '耗时',
      ...
      openai: {
        ...
        codexQuotaOverdraftDesc: '控制当前账号是否可以在 Codex 额度用尽后继续透支，以及哪些账号类型可以参与透支。',
        codexQuotaOverdraftOAuth: '允许 OAuth 参与',
        codexQuotaOverdraftOAuthDesc: 'OAuth 账号默认参与透支。',
        codexQuotaOverdraftSetupToken: '允许 Setup Token 参与',
        codexQuotaOverdraftSetupTokenDesc: '开启后，Setup Token 账号也可以在当前分组透支后接力。',
