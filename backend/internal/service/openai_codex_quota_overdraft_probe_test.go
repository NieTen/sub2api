...
func (r *codexOverdraftProbeRepoStub) ClearCodexQuotaOverdraftPauseIfState(
	_ context.Context,
	_ int64,
	cycleKey string,
	status string,
	...
	persisted, ok := codexQuotaOverdraftStateFromAccount(r.account)
	if !ok || persisted.CycleKey != cycleKey || persisted.Status != status {
		return false, false, nil
	}
	clearedRateLimit := observedRateLimitReset != nil &&
		r.account.RateLimitResetAt != nil &&
		r.account.RateLimitResetAt.Equal(observedRateLimitReset.UTC())
	if clearedRateLimit {
		r.clearLimitCalls++
		r.account.RateLimitedAt = nil
		r.account.RateLimitResetAt = nil
	}
	clearedTemp := expectedTempReason != "" &&
		r.account.TempUnschedulableReason == expectedTempReason
	if clearedTemp {
		r.clearTempCalls++
		r.account.TempUnschedulableUntil = nil
		r.account.TempUnschedulableReason = ""
...
func TestCodexQuotaOverdraftAvailableStateClearsOnlyObservedRateLimit(t *testing.T) {
	...
	for _, status := range []string{codexQuotaOverdraftProbePassed, codexQuotaOverdraftProbeRecovered} {
		t.Run(status, func(t *testing.T) {
			for _, tt := range []struct {
				...
				{name: "匹配旧429", current: now.Add(time.Hour), observed: now.Add(time.Hour), wantClear: true},
				{name: "不匹配新429", current: now.Add(2 * time.Hour), observed: now.Add(time.Hour), wantClear: false},
			} {
				t.Run(tt.name, func(t *testing.T) {
					account := newCodexOverdraftProbeTestAccount(now)
					account.RateLimitedAt = codexQuotaOverdraftTimePtr(now.Add(-time.Minute))
					account.RateLimitResetAt = codexQuotaOverdraftTimePtr(tt.current)
					repo := &codexOverdraftProbeRepoStub{account: account}
					...
					state := &CodexQuotaOverdraftProbeState{
						...
						ObservedRateLimit: codexQuotaOverdraftTimePtr(tt.observed),
					}
					account.Extra[CodexQuotaOverdraftProbeExtraKey] = state
