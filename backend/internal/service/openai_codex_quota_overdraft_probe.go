...
// CodexQuotaOverdraftProbeState 持久化在 accounts.extra 中，
// 用于让额度周期决策跨重启保留，并通过账号用量接口展示。
...
type codexQuotaOverdraftPauseClearer interface {
	ClearCodexQuotaOverdraftPauseIfState(context.Context, int64, string, string, string, *time.Time) (bool, bool, error)
}

// CodexQuotaOverdraftCoordinator 实现有界真实请求复核门，
// 用于在确认额度耗尽前避免过早停用账号。
...
// ObserveAccount 在持久化 5h/7d 快照首次达到 100% 时启动探测，
// 并在额度窗口恢复后关闭旧透支周期。
...
// ObserveBusinessSuccess 将真实携带透支载荷且成功的业务请求视为比合成探测更强的可用证据。
...
// persistNonFailedState 保持当前周期已经确认的额度失败为终态，
// 即使并发成功请求或探测稍后完成也不会覆盖。
