...
> 本分支采用“源仓库母版本 + 定制子版本”的四段版本号。合并与发布要求见[版本编号与发布规则](docs/VERSION_POLICY_CN.md)。
...
- **管理后台** - Web 界面进行监控和管理
- **机器人与群发** - 后台统一管理 Telegram 机器人、图文工单、按余额和成功充值记录筛选的批量邮件，以及个人入群邀请和 VIP 社群（[配置指南](SUPPORT_TICKETS_CN.md)）
...
```bash
curl -sSL https://raw.githubusercontent.com/NieTen/sub2api/main/deploy/install.sh | sudo bash
```
...
# 卸载
curl -sSL https://raw.githubusercontent.com/NieTen/sub2api/main/deploy/install.sh | sudo bash -s -- uninstall -y
```
...
# 下载并运行部署准备脚本
curl -sSL https://raw.githubusercontent.com/NieTen/sub2api/main/deploy/docker-deploy.sh | bash
...
```bash
# 1. 克隆仓库
git clone https://github.com/NieTen/sub2api.git
cd sub2api/deploy
...
<a href="https://star-history.dera.page/#NieTen/sub2api&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://star-history.dera.page/svg?repos=NieTen/sub2api&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://star-history.dera.page/svg?repos=NieTen/sub2api&type=Date" />
   <img alt="Star History Chart" src="https://star-history.dera.page/svg?repos=NieTen/sub2api&type=Date" />
 </picture>
</a>
