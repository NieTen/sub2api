...
class ReleaseMatrixTest(unittest.TestCase):
    ...
    def test_publication_config_has_no_compilation_or_docker_work(self):
        for simple in (False, True):
            with self.subTest(simple=simple):
                ...
                for key, value in original['release'].items():
                    self.assertEqual(data['release'][key], value)
                if simple:
                    self.assertTrue(data['checksum']['disable'])
                    self.assertTrue(data['release'].get('disable') or data['release'].get('skip_upload'))
                else:
    ...
    def test_four_part_release_preserves_version_and_target_names(self):
        release.VERSION_FILE.write_text('0.2.8.1\n')
        with patch.dict(os.environ, {'GITHUB_OUTPUT': 'outputs'}), patch.object(subprocess, 'check_output', return_value='a' * 40 + '\n'):
            release.plan(argparse.Namespace(ref='v0.2.8.1', dry_run=False, simple=False))
        output = dict(line.split('=', 1) for line in Path('outputs').read_text().splitlines())
        ...
        self.assertEqual(release.archive_name('0.2.8.1', {'goos': 'linux', 'goarch': 'amd64'}), 'sub2api_0.2.8.1_linux_amd64.tar.gz')
        self.assertEqual(release.archive_name('0.2.8.1', {'goos': 'windows', 'goarch': 'amd64'}), 'sub2api_0.2.8.1_windows_amd64.zip')

    def test_publication_rejects_version_file_mismatch(self):
        with patch.object(subprocess, 'check_output', return_value='a' * 40 + '\n'):
            with self.assertRaisesRegex(ValueError, 'VERSION does not match'):
                release.plan(argparse.Namespace(ref='v0.2.8.1', dry_run=False, simple=False))

    def test_archive_rejects_invalid_or_unsafe_version(self):
        target = {'goos': 'linux', 'goarch': 'amd64'}
        for version in ('0.2.8.1.2', '../0.2.8.1', '0.2.8.1/other', '0.2.8.1\n'):
